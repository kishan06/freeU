...
PrivateEquityDetailsModel ? privateequitydetailsobj;

class PrivateEquityDetails {
  PrivateEquityDetails();

  Future<ResponseData<dynamic>> PrivateEquityDetailsAPI(slug) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    final response = await NetworkApi().getApi(ApiUrls.PrivateEquityDetails + "/${slug}");
    ...
    if (response.status == ResponseStatus.SUCCESS) {
      ...
      if (responseData.isNotEmpty) {
        privateequitydetailsobj =
            PrivateEquityDetailsModel.fromJson(responseData);
            print("data is $privateequitydetailsobj");
      } else {
