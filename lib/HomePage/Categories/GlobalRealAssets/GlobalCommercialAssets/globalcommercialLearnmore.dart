...
class GlobalCommercialLearnmore extends StatefulWidget {
  const GlobalCommercialLearnmore({super.key});

  @override
  State<GlobalCommercialLearnmore> createState() =>
      _GlobalCommercialLearnmoreState();
}

class _GlobalCommercialLearnmoreState extends State<GlobalCommercialLearnmore> {
...
For HNIs who are looking to diversify their portfolios and increase long-term returns on their investments while willing to take a considerable risk, Bonds seem like decent investment alternatives. But for those who can take low to high risk, mutual funds are the way to go. The decision between alternative investment funds and mutual funds should be based on your investment objectives, available capital, and long-term intentions.
 '''
  ...
  @override
  void initState() {
    myfuture = FAQ2().GlobalCommercialLearn();
    super.initState();
  }
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: CustomNextButton(
          ...
          ontap: () {
            Get.toNamed("/globalcommercialproducts");
          },
        ),
      ),
      body: FutureBuilder(
          ...
          Expanded(
            child: ListView.separated(
                itemBuilder: (context, index) {
                  return globalcommerciallearn(
                    globalcommercial!.data![index].faqQuestion ?? "",
                    globalcommercial!.data![index].faqAnswer ?? "",
                  );
                },
                separatorBuilder: (context, index) {
                  return sizedBoxHeight(20.h);
                },
                itemCount: globalcommercial!.data!.length),
          ),
        ],
      ),
    );
  ...
  Widget globalcommerciallearn(String question, String ans) {
    return Container(
