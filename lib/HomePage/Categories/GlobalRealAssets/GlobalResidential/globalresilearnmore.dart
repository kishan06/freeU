...
class GlobalResidentLearnmore extends StatefulWidget {
  const GlobalResidentLearnmore({super.key});

  @override
  State<GlobalResidentLearnmore> createState() => _GlobalResidentLearnmoreState();
}

class _GlobalResidentLearnmoreState extends State<GlobalResidentLearnmore> {
...
For HNIs who are looking to diversify their portfolios and increase long-term returns on their investments while willing to take a considerable risk, Bonds seem like decent investment alternatives. But for those who can take low to high risk, mutual funds are the way to go. The decision between alternative investment funds and mutual funds should be based on your investment objectives, available capital, and long-term intentions.
 '''
  ...
  @override
  void initState() {
    myfuture = FAQ2().GlobalResidentialLearn();
    super.initState();
  }
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: CustomNextButton(
          ...
          ontap: () {
            Get.toNamed("/globalresidenproducts");
          },
        ),
      ),
      body: 
          ...
          Expanded(
            child: ListView.separated(
                itemBuilder: (context, index) {
                  return gloabalresidentlearn(
                    globalresident!.data![index].faqQuestion ?? "",
                    globalresident!.data![index].faqAnswer ?? "",
                  );
                },
                separatorBuilder: (context, index) {
                  return sizedBoxHeight(20.h);
                },
                itemCount: globalresident!.data!.length),
          ),
        ],
      ),
    );
  ...
  Widget gloabalresidentlearn(String question, String ans) {
    return Container(
