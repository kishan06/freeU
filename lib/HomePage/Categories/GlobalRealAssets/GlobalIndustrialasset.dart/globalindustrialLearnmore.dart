...
class GlobalIndustrialLearnmore extends StatefulWidget {
  const GlobalIndustrialLearnmore({super.key});

  @override
  State<GlobalIndustrialLearnmore> createState() =>
      _GlobalIndustrialLearnmoreState();
}

class _GlobalIndustrialLearnmoreState extends State<GlobalIndustrialLearnmore> {
...
For HNIs who are looking to diversify their portfolios and increase long-term returns on their investments while willing to take a considerable risk, Bonds seem like decent investment alternatives. But for those who can take low to high risk, mutual funds are the way to go. The decision between alternative investment funds and mutual funds should be based on your investment objectives, available capital, and long-term intentions.
 '''
  ...
  @override
  void initState() {
    myfuture = FAQ2().GlobalIndustrialLearn();
    super.initState();
  }
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: CustomNextButton(
          text: 'View Real Assets',
          ontap: () {
            Get.toNamed("/globalindustrialproducts");
          },
        ),
      ),
      body: FutureBuilder(
          ...
          Expanded(
            child: ListView.separated(
                itemBuilder: (context, index) {
                  return globalindustriallearn(
                    globalindustrial!.data![index].faqQuestion ?? "",
                    globalindustrial!.data![index].faqAnswer ?? "",
                  );
                },
                separatorBuilder: (context, index) {
                  return sizedBoxHeight(20.h);
                },
                itemCount: globalindustrial!.data!.length),
          ),
        ],
      ),
    );
  ...
  Widget globalindustriallearn(String question, String ans) {
    return Container(
