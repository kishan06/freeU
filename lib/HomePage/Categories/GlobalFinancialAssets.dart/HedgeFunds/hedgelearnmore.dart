...
class HedgeFundsLearnmore extends StatefulWidget {
  const HedgeFundsLearnmore({super.key});

  @override
  State<HedgeFundsLearnmore> createState() => _HedgeFundsLearnmoreState();
}

class _HedgeFundsLearnmoreState extends State<HedgeFundsLearnmore> {
...
For HNIs who are looking to diversify their portfolios and increase long-term returns on their investments while willing to take a considerable risk, Bonds seem like decent investment alternatives. But for those who can take low to high risk, mutual funds are the way to go. The decision between alternative investment funds and mutual funds should be based on your investment objectives, available capital, and long-term intentions.
 '''
  ...
  @override
  void initState() {
    myfuture = FAQ2().GlobalHedgefunds();
    super.initState();
  }
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: CustomNextButton(
          text: 'View Hedge Funds',
          ontap: () {
            Get.toNamed("/hedgefundsproducts");
          },
        ),
      ),
      body: FutureBuilder(
          ...
          Expanded(
            child: ListView.separated(
                itemBuilder: (context, index) {
                  return globalhedgelearn(
                    globalhedgefunds!.data![index].faqQuestion ?? "",
                    globalhedgefunds!.data![index].faqAnswer ?? "",
                  );
                },
                separatorBuilder: (context, index) {
                  return sizedBoxHeight(20.h);
                },
                itemCount: globalhedgefunds!.data!.length),
          ),
        ],
      ),
    );
  ...
  Widget globalhedgelearn(String question, String ans) {
    return Container(
