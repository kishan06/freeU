...
class BondsLearnmore extends StatefulWidget {
  const BondsLearnmore({super.key});

  @override
  State<BondsLearnmore> createState() => _BondsLearnmoreState();
}

class _BondsLearnmoreState extends State<BondsLearnmore> {
...
For HNIs who are looking to diversify their portfolios and increase long-term returns on their investments while willing to take a considerable risk, Bonds seem like decent investment alternatives. But for those who can take low to high risk, mutual funds are the way to go. The decision between alternative investment funds and mutual funds should be based on your investment objectives, available capital, and long-term intentions.
 '''
  ...
  @override
  void initState() {
    myfuture = FAQ2().GlobalBondsLearnmore();
    super.initState();
  }
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: CustomNextButton(
          text: 'View Bonds Assets',
          ontap: () {
            Get.toNamed("/bondsassetsmain");
          },
        ),
      ),
      body: FutureBuilder(
          ...
          Expanded(
            child: ListView.separated(
                itemBuilder: (context, index) {
                  return globalbonds(
                    globalbondsmore!.data![index].faqQuestion ?? "",
                    globalbondsmore!.data![index].faqAnswer ?? "",
                  );
                },
                separatorBuilder: (context, index) {
                  return sizedBoxHeight(20.h);
                },
                itemCount: globalbondsmore!.data!.length),
          ),
        ],
      ),
    );
  ...
  Widget globalbonds(String question, String ans) {
    return Container(
