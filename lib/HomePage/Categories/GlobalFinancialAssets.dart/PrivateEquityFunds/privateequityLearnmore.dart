...
class PrivateEquityLearnmore extends StatefulWidget {
  const PrivateEquityLearnmore({super.key});

  @override
  State<PrivateEquityLearnmore> createState() => _PrivateEquityLearnmoreState();
}

class _PrivateEquityLearnmoreState extends State<PrivateEquityLearnmore> {
...
For HNIs who are looking to diversify their portfolios and increase long-term returns on their investments while willing to take a considerable risk, Bonds seem like decent investment alternatives. But for those who can take low to high risk, mutual funds are the way to go. The decision between alternative investment funds and mutual funds should be based on your investment objectives, available capital, and long-term intentions.
 '''
  ...
  @override
  void initState() {
    myfuture = FAQ2().GlobalPrivateEquiFunds();
    super.initState();
  }
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: CustomNextButton(
          text: 'View Private Equity Assets',
          ontap: () {
            Get.toNamed("/privateequityproducts");
          },
        ),
      ),
      body: FutureBuilder(
          ...
          Expanded(
            child: ListView.separated(
                itemBuilder: (context, index) {
                  return globalprivatelearn(
                    globalprivateequityfunds!.data![index].faqQuestion ?? "",
                    globalprivateequityfunds!.data![index].faqAnswer ?? "",
                  );
                },
                separatorBuilder: (context, index) {
                  return sizedBoxHeight(20.h);
                },
                itemCount: globalprivateequityfunds!.data!.length),
          ),
        ],
      ),
    );
  ...
  Widget globalprivatelearn(String question, String ans) {
    return Container(
