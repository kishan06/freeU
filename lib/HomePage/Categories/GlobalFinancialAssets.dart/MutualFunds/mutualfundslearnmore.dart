...
class MutualFundsLearnmore extends StatefulWidget {
  const MutualFundsLearnmore({super.key});

  @override
  State<MutualFundsLearnmore> createState() => _MutualFundsLearnmoreState();
}

class _MutualFundsLearnmoreState extends State<MutualFundsLearnmore> {
...
For HNIs who are looking to diversify their portfolios and increase long-term returns on their investments while willing to take a considerable risk, Bonds seem like decent investment alternatives. But for those who can take low to high risk, mutual funds are the way to go. The decision between alternative investment funds and mutual funds should be based on your investment objectives, available capital, and long-term intentions.
 '''
  ...
  @override
  void initState() {
    myfuture = FAQ2().GlobalMutualFundsLearn();
    super.initState();
  }
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: CustomNextButton(
          text: 'View Mutual Assets',
          ontap: () {
            Get.toNamed("/mutualfunfsproduct");
          },
        ),
      ),
      body: FutureBuilder(
          ...
          Expanded(
            child: ListView.separated(
                itemBuilder: (context, index) {
                  return globalmutuallearn(
                    globalmutualfunds!.data![index].faqQuestion ?? "",
                    globalmutualfunds!.data![index].faqAnswer ?? "",
                  );
                },
                separatorBuilder: (context, index) {
                  return sizedBoxHeight(20.h);
                },
                itemCount: globalmutualfunds!.data!.length),
          ),
        ],
      ),
    );
  ...
  Widget globalmutuallearn(String question, String ans) {
    return Container(
