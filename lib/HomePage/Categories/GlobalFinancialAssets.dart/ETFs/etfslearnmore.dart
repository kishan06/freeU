...
class ETFsLearnmore extends StatefulWidget {
  const ETFsLearnmore({super.key});

  @override
  State<ETFsLearnmore> createState() => _ETFsLearnmoreState();
}

class _ETFsLearnmoreState extends State<ETFsLearnmore> {
...
For HNIs who are looking to diversify their portfolios and increase long-term returns on their investments while willing to take a considerable risk, Bonds seem like decent investment alternatives. But for those who can take low to high risk, mutual funds are the way to go. The decision between alternative investment funds and mutual funds should be based on your investment objectives, available capital, and long-term intentions.
 '''
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: CustomNextButton(
          text: 'View ETFs Funds',
          ontap: () {
            Get.toNamed("/etfsproducts");
          },
        ),
      ),
      body: FutureBuilder(
          ...
          Expanded(
            child: ListView.separated(
                itemBuilder: (context, index) {
                  return globaletfslearn(
                    globaletfslearnmore!.data![index].faqQuestion ?? "",
                    globaletfslearnmore!.data![index].faqAnswer ?? "",
                  );
                },
                separatorBuilder: (context, index) {
                  return sizedBoxHeight(20.h);
                },
                itemCount: globaletfslearnmore!.data!.length),
          ),
        ],
      ),
    );
  ...
  Widget globaletfslearn(String question, String ans) {
    return Container(
