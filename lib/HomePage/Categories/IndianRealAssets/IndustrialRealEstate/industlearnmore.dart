...
class IndustrialLearnmore extends StatefulWidget {
  const IndustrialLearnmore({super.key});

  @override
  State<IndustrialLearnmore> createState() => _IndustrialLearnmoreState();
}

class _IndustrialLearnmoreState extends State<IndustrialLearnmore> {
...
For HNIs who are looking to diversify their portfolios and increase long-term returns on their investments while willing to take a considerable risk, Bonds seem like decent investment alternatives. But for those who can take low to high risk, mutual funds are the way to go. The decision between alternative investment funds and mutual funds should be based on your investment objectives, available capital, and long-term intentions.
 '''
  ...
  @override
  void initState() {
    myfuture = FAQ2().IndianIndustrialLearn();
    super.initState();
  }
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: CustomNextButton(
          ...
          ontap: () {
            Get.toNamed("/industrialproducts");
          },
        ),
      ),
      body: FutureBuilder(
      ...
      child: Column(
        ...
        children: [
          ...
          Expanded(
            child: ListView.separated(
                itemBuilder: (context, index) {
                  return indianindustriallearn(
                    indianindustrial!.data![index].faqQuestion ?? "",
                    indianindustrial!.data![index].faqAnswer ?? "",
                  );
                },
                separatorBuilder: (context, index) {
                  return sizedBoxHeight(20.h);
                },
                itemCount: indianindustrial!.data!.length),
          ),
        ],
      ),
    );
  ...
  Widget indianindustriallearn(String question, String ans) {
    return Container(
