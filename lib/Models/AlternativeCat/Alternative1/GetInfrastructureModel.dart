class InfrastructureFundModel {
  List<Data>? data;

  InfrastructureFundModel({this.data});

  InfrastructureFundModel.fromJson(Map<String, dynamic> json) {
    if (json['data'] != null) {
      data = <Data>[];
      json['data'].forEach((v) {
...
class AlternativeInvestmentFund {
  ...
  Null? regionsCovered;
  String? isinCode;
  Null? focusedRealEstateSectors;
  Null? reraCompliedProperty;
  Null? returnOnInvestment;
  Null? valuationPerSector;
  Null? focusedFunds;
  Null? tradingStrategy;
  Null? involvedInShortSelling;
